...
export default function MapeoCartera({ cartera, initialColumnas = [] }: Props) {
  ...
  const [columnas, setColumnas] = useState<string[]>(() => {
    ...
    return ((cartera.mapItems as MapItem[]) ?? []).map(
      (m) => m.columna_name_origen,
    );
  });
  ...
  const [mapping, setMapping] = useState<Record<string, string>>(() => {
    ...
    if (existing.length > 0)
      return Object.fromEntries(
        existing.map((m) => [m.columna_name_origen, m.campo_operaciones]),
      );
  ...
  const handleSave = async () => {
    ...
    const mapItems: MapItem[] = columnas
      .filter((c) => mapping[c])
      .map((c) => ({ columna_name_origen: c, campo_operaciones: mapping[c] }));
  ...
  const handleIniciarCarga = async () => {
    ...
    const { total, nuevos, otrosStatus, error } =
      await getCarteraOperacionesStatusAction(cartera.id);
    if (error) {
      toast.error(error);
      return;
    }
  ...
  const handleConfirmarRecarga = async () => {
    ...
    if (eliminados > 0)
      toast.success(`${eliminados} registros 'Nuevo' eliminados`);
    setConfirmando(false);
    setStatusInfo(null);
    await ejecutarCarga();
  };
  ...
  const ejecutarCarga = async () => {
    setCargando(true);
    const { insertadas, omitidas, error } = await cargarOperacionesAction(
      cartera.id,
    );
    setCargando(false);
    if (error) {
    ...
    setResultado({ insertadas, omitidas });
              ...
              <p className="text-sm font-medium text-blue-800">
                ...
                {asignadas < columnas.length && (
                  <span className="ml-2 text-xs text-amber-600">
                    ({columnas.length - asignadas} se ignorarán)
                  </span>
                )}
              </p>
              {mapGuardado && (
          ...
          <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
            <table className="min-w-full divide-y divide-gray-100 text-sm">
              <thead className="bg-gray-50">
                <tr>
                  <th className="w-8 px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-400">
                    #
                  </th>
              ...
              <tbody className="divide-y divide-gray-50">
                {columnas.map((col, i) => (
                  <tr
                    key={col}
                    className={mapping[col] ? "bg-emerald-50/30" : ""}
                  >
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 space-y-4">
          <div>
            <h3 className="text-sm font-semibold text-gray-900">
              Cargar operaciones en base de datos
            </h3>
            <p className="text-xs text-gray-500 mt-1">
              Se insertan en tabla{" "}
              <code className="bg-gray-100 px-1 rounded">operaciones</code> con
              status <strong>Nuevo</strong> y fecha de hoy.
            </p>
          </div>
          ...
          {confirmando && statusInfo && (
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 space-y-3">
              <p className="text-sm font-semibold text-amber-800">
                ⚠ Ya existen registros en esta cartera
              </p>
              <div className="text-xs text-amber-700 space-y-0.5">
                <p>
                  Total registros: <strong>{statusInfo.total}</strong>
                </p>
                <p>
                  Con status <em>Nuevo</em> (se eliminarán):{" "}
                  <strong className="text-red-600">{statusInfo.nuevos}</strong>
                </p>
                <p>
                  Con otro status (se conservarán):{" "}
                  <strong className="text-emerald-600">
                    {statusInfo.otrosStatus}
                  </strong>
                </p>
              </div>
              <p className="text-xs text-amber-700">
                Se eliminarán los <strong>{statusInfo.nuevos}</strong> registros
                con status <em>Nuevo</em> y se cargarán los del Excel. Los
                registros con otro status <strong>no se tocan</strong>.
              </p>
              {statusInfo.nuevos === 0 && (
                <p className="text-xs text-emerald-700 font-medium">
                  No hay registros con status Nuevo. Se añadirán los nuevos sin
                  eliminar nada.
                </p>
              )}
          ...
          {resultado && (
            <p className="text-sm font-semibold text-emerald-800">
              ✓ {resultado.insertadas} operaciones cargadas
              {resultado.omitidas > 0 && (
                <span className="ml-2 text-amber-600">
                  · {resultado.omitidas} omitidas (main_key duplicada)
                </span>
              )}
            </p>
          )}
