...
export default function ProvinciasMunicipiosSelectEnrichment() {
  const {
    register,
    setValue,
    getValues,
  ...
  useEffect(() => {
    ...
    loadedCodeRef.current = code;
    setValue('municipio', '');
    setValue('municipioId', undefined);
    loadMunicipios(code);
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <div>
        ...
        <select
          ...
          {...register('municipio')}
          onChange={(e) => {
            // Al seleccionar municipio, guardamos también el municipioId
            const selected = municipioOptions.find((m) => m.nombre === e.target.value);
            setValue('municipio', e.target.value);
            setValue('municipioId', selected ? selected.municipioId : undefined);
          }}
        >
          {loading ? (
