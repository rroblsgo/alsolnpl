...
export default async function ClienteDetailPage({ params }: Props) {
  ...
  const [cliente, tasks] = await Promise.all([
    clienteService.getCliente(clienteId),
    taskService.listTasksByCliente(clienteId),
  ]);
  ...
  const activosLabels = cliente.activosInteresado
    .map((a) => ACTIVOS_INTERESADO_OPTIONS.find((o) => o.value === a)?.label ?? a)
    .join(', ');
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <Section title="C. Perfil inversor">
        <dl className="grid grid-cols-2 gap-4 sm:grid-cols-3">
          <DataRow label="Perfil" value={cliente.perfilInversor ? CLIENTE_PERFIL_LABELS[cliente.perfilInversor] : null} />
          <DataRow label="Ocupación" value={cliente.ocupacionPrincipal ? CLIENTE_OCUPACION_LABELS[cliente.ocupacionPrincipal] : null} />
          <DataRow label="Capital a invertir" value={cliente.rangoCapitalInvertir ? CLIENTE_RANGO_CAPITAL_LABELS[cliente.rangoCapitalInvertir] : null} />
          {activosLabels && (
            <div className="col-span-2 sm:col-span-3">
              <dt className="text-xs uppercase tracking-wide text-gray-400">Activos de interés</dt>
              <dd className="mt-1 flex flex-wrap gap-1">
                {cliente.activosInteresado.map((a) => (
                  <span key={a} className="rounded-full bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700">
                    {ACTIVOS_INTERESADO_OPTIONS.find((o) => o.value === a)?.label ?? a}
                  </span>
                ))}
              </dd>
            </div>
          ...
          {cliente.experienciaPreviaDetalle && (
            <div className="col-span-2 sm:col-span-3">
              <DataRow label="Experiencia previa" value={cliente.experienciaPreviaDetalle} />
            </div>
          )}
          {cliente.informadoNplDetalle && (
            <div className="col-span-2 sm:col-span-3">
              <DataRow label="Cómo conoce los NPL" value={cliente.informadoNplDetalle} />
            </div>
          )}
        </dl>
      </Section>
      ...
      <Section title="D. Gestión interna">
        <dl className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <DataRow label="Estado" value={CLIENTE_ESTADO_LABELS[cliente.estado]} />
          <DataRow label="Fuente captación" value={cliente.fuenteCaptacion ? CLIENTE_FUENTE_LABELS[cliente.fuenteCaptacion] : null} />
          <DataRow label="RGPD" value={cliente.consentimientoRgpd ? 'Consentimiento otorgado' : 'Sin consentimiento'} />
          {cliente.fechaConsentimiento && (
            <DataRow
              label="Fecha consentimiento"
              value={new Date(cliente.fechaConsentimiento).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}
            />
          )}
          <DataRow
            label="Alta"
            value={new Date(cliente.createdAt).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}
          />
        </dl>
        {cliente.notas && (
          <div className="mt-4 border-t pt-4">
            <p className="text-xs uppercase tracking-wide text-gray-400 mb-1">Notas internas</p>
      ...
      <Section title="E. Tareas / Actuaciones">
        <div className="mb-3 flex items-center justify-between">
          <p className="text-sm text-gray-500">
            {tasks.length > 0
              ? `${tasks.length} tarea${tasks.length !== 1 ? 's' : ''} vinculada${tasks.length !== 1 ? 's' : ''}`
              : 'Sin tareas vinculadas'}
          </p>
          <Link
            href={`/dashboard/tasks/create?clienteId=${cliente.id}`}
